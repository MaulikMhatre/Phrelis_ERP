...
import { Clock, Search, Activity, Calendar, ArrowLeft, Filter, AlertCircle, FileText, Scissors } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { endpoints } from '@/utils/api';
...
export default function HistoryPage() {
  // NEW: State to toggle between Clinical and Surgery views
  const [activeTab, setActiveTab] = useState<"CLINICAL" | "SURGERY">("CLINICAL");
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const url = activeTab === "CLINICAL"
          ? endpoints.historyByDate(selectedDate)
          : endpoints.historySurgery;
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-white/10 pb-8">
          <div>
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-3 mb-2">
              <div className="p-3 bg-indigo-500/10 rounded-xl border border-indigo-500/20 shadow-[0_0_20px_rgba(99,102,241,0.1)]">
                {activeTab === "CLINICAL" ? <Clock className="text-indigo-400 w-6 h-6" /> : <Scissors className="text-purple-400 w-6 h-6" />}
              </div>
              <h1 className="text-4xl font-black tracking-tight text-white uppercase">
                {activeTab === "CLINICAL" ? "Clinical Logs" : "Surgical Logs"}
              </h1>
            </motion.div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-white/5 bg-white/[0.02]">
                    <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Timestamp</th>
                    <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Patient Identity</th>
                    <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{activeTab === "CLINICAL" ? "Status" : "Surgeon"}</th>
                    <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{activeTab === "CLINICAL" ? "Triage Level" : "Duration"}</th>
                    <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">{activeTab === "CLINICAL" ? "Acuity" : "Overtime"}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {filteredHistory.length > 0 ? (
                    filteredHistory.map((row, i) => (
                      <motion.tr key={row.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="group hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="p-6">
                          ...
                          <div className="text-[10px] font-bold text-slate-600 uppercase tracking-wider mt-1">Age: {row.patient_age}</div>
                        </td>
                        <td className="p-6">
                          {activeTab === "CLINICAL" ? (
                            <div className="flex items-center gap-2">
