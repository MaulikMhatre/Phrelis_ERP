...
// import { Clock, Scissors, Activity, User, Calendar } from "lucide-react";

// export default function CombinedHistoryPage() {
//     // State to toggle between 'GENERAL' (ICU/ER) and 'SURGERY'
...
//             ? `/api/history/day/${new Date().toISOString().split('T')[0]}` 
//             : `/api/history/surgery`;
...
//                     <p className="text-slate-400">Viewing {view === "GENERAL" ? "General Admissions" : "Surgical Records"}</p>
//                 </div>

//                 <div className="flex bg-white/5 p-1 rounded-xl border border-white/10">
//                     <button 
//                         onClick={() => setView("GENERAL")}
//                         className={`px-6 py-2 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${view === "GENERAL" ? "bg-blue-600 text-white shadow-lg" : "text-slate-400 hover:text-white"}`}
//                     >
//                         <Activity size={16} /> Patient History
//                     </button>
//                     <button 
//                         onClick={() => setView("SURGERY")}
//                         className={`px-6 py-2 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${view === "SURGERY" ? "bg-purple-600 text-white shadow-lg" : "text-slate-400 hover:text-white"}`}
//                     >
//                         <Scissors size={16} /> Surgery History
//                     </button>
//                 </div>
//             </div>
...
//                             <th className="p-4">{view === "GENERAL" ? "Condition" : "Surgeon"}</th>
//                             <th className="p-4">Unit/Room</th>
//                             <th className="p-4">{view === "GENERAL" ? "ESI Level" : "Duration"}</th>
...
//                                 <td className="p-4 font-bold">{item.patient_name}</td>
//                                 <td className="p-4">
//                                     {view === "GENERAL" ? (
//                                         <span className="text-slate-300">{item.condition}</span>
//                                     ) : (
//                                         <span className="text-purple-400 font-medium">{item.surgeon_name}</span>
//                                     )}
//                                 </td>
//                                 <td className="p-4 text-slate-400">{view === "GENERAL" ? item.bed_id : item.room_id}</td>
//                                 <td className="p-4">
//                                     {view === "GENERAL" ? (
//                                         <span className={`px-2 py-1 rounded text-xs font-black ${item.esi_level <= 2 ? 'bg-red-500/20 text-red-400' : 'bg-blue-500/20 text-blue-400'}`}>
//                                             ESI {item.esi_level}
//                                         </span>
//                                     ) : (
//                                         <div className="flex items-center gap-2">
//                                             <span className="text-slate-300">{item.total_duration_minutes}m</span>
//                                             {item.overtime_minutes > 0 && <span className="text-red-500 text-xs">+{item.overtime_minutes}m</span>}
//                                         </div>
//                                     )}
//                                 </td>
//                                 <td className="p-4 text-right text-slate-500 text-sm">
//                                     {new Date(item.timestamp || item.end_time).toLocaleString()}
//                                 </td>
//                             </tr>
//                         ))}
//                     </tbody>
